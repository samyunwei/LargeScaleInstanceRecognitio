...
class EzSIFT(DescriptorBase):
    def __init__(self):
        ...
        if os.name == "nt":
            self.program = "third_tools/ezsift_win"
        elif os.name == "posix":
            self.program = "third_tools/ezsift_unix"
        return
    ...
    def describe(self, img):
        ...
        cv2.imwrite('data/cache.pgm', img)
        cv2.imwrite('data/cache.jpg', img)

        p = subprocess.Popen([self.program, 'data/cache.pgm'], stdout=subprocess.PIPE,
    ...
    def read_keypoints_file(self):
        path = "data/cache.pgm_sift_key.key"
